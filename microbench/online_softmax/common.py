...
def verify_numeric(inp, act_max, act_sum, ref_max=None, ref_sum=None):
    ...
    tol = 1e-3
    assert torch.allclose(ref_max, act_max, atol=tol, rtol=tol)
    assert torch.allclose(ref_sum, act_sum, atol=tol, rtol=tol)
...
def benchmark(label, call, args, ref_max=None, ref_sum=None):
    ...
    verify_numeric(*args, ref_max, ref_sum)
    print(f"{label:20s} {ms:.3f}ms PASS")
