...
if True: # triton
    ...
    triton_bwd_out = triton_attn_bwd(dY, Q, K, V, None, rowmax, rowsum)
    assert_close(ref_bwd_out, triton_bwd_out)
    triton_bwd_ms = do_bench(lambda: triton_attn_bwd(dY, Q, K, V, None, rowmax, rowsum))
    print(f"{triton_bwd_ms=}")
