"""Tests for a first-fit MemoryAllocator.
...
allocate(size) -> int:
    Reserve `size` consecutive free units using the *first-fit* strategy:
    scan the maximal runs of free units in increasing start-index order and
    pick the *first* run whose length is >= size (its size relative to other
    runs is irrelevant). Reserve the first `size` units of that run and return
    its start index. Return -1 if no run is large enough (or size <= 0).
...
# --------------------------------------------------------------------------
# First-fit selection (the heart of this allocator)
# --------------------------------------------------------------------------

def test_first_fit_picks_first_sufficient_hole():
    """A size-1 request must go to the first (lowest-address) sufficient hole.

    Best-fit would pick the tighter size-1 hole at index 9; first-fit must
    pick the earlier size-2 hole at index 3.
    """
    ...
    assert a.allocate(1) == 3   # first fit chooses the earlier hole


def test_first_fit_uses_earlier_larger_hole_not_a_tighter_later_one():
    """The earlier hole is *larger* than needed; first-fit still takes it."""
    a = MemoryAllocator(10)
    ...
    assert a.allocate(3) == 0   # first fit: the size-4 hole at 0, not size-3 at 5
    assert a.allocate(1) == 3   # leftover of that hole (it was split)


def test_first_fit_scans_in_address_order():
    a = MemoryAllocator(9)
    a.allocate(2)               # 0..1
    a.allocate(2)               # 2..3
    a.allocate(2)               # 4..5
    ...
    # two equal holes -> the first one wins
    assert a.allocate(2) == 0


def test_first_fit_exact_fit_consumes_whole_hole():
    a = MemoryAllocator(10)
    a.allocate(3)               # 0..2
    b = a.allocate(2)           # 3..4
...
def test_scenario_mixed_operations():
    ...
    assert a.allocate(1) == 2   # first fit -> earliest hole at index 2
