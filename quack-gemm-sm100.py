...
from cutlass.pipeline import PipelineAsync, PipelineState, PipelineUserType, NamedBarrier
...
def GemmSm100_get_scheduler_params(
    ...
    problem_shape_ntile_mnl = (
        cute.ceil_div(mA.shape[0], self.mma_tiler[0]),
        cute.ceil_div(mB.shape[0], self.mma_tiler[1]),
        mA.shape[2],
    )
...
def GemmSm100_make_ab_pipeline(
    self,
...
def GemmSm100_make_acc_pipeline(
    self, cluster_layout_vmnk: cute.Layout, acc_pipeline_mbar_ptr: cute.Pointer
) -> cutlass.pipeline.PipelineAsync:
    producer_group = cutlass.pipeline.CooperativeGroup(cutlass.pipeline.Agent.Thread)
    consumer_group = cutlass.pipeline.CooperativeGroup(cutlass.pipeline.Agent.Thread, self.num_epi_warps)
...
def GemmSm100_make_epi_store_pipeline(self):
...
@staticmethod
def GemmSm100__compute_num_tmem_alloc_cols(
    tiled_mma: cute.TiledMma,
    ...
    return cutlass.utils.get_num_tmem_alloc_cols(tCtAcc_fake)
...
def copy_utils_tma_get_copy_fn(
    ...
    smem_last_rank = cute.rank(smem_tensor) - 1
    gmem_last_rank = cute.rank(gmem_tensor) - 1
    s, g = cpasync.tma_partition(
        atom,
        cta_coord,
        cta_layout,
        cute.group_modes(smem_tensor, 0, smem_last_rank),
        cute.group_modes(gmem_tensor, 0, gmem_last_rank),
    )
    src, dst = (s, g) if src_is_smem else (g, s)

    def copy_tma(src_idx, dst_idx, **new_kwargs):
        cute.copy(atom, src[None, src_idx], dst[None, dst_idx], **new_kwargs)

    return copy_tma

@cute.jit
def GemmSm100_load_AB(
    self,
    ab_pipeline: PipelineAsync,
    ab_producer_state: PipelineState,
    copy_A: Callable,
    copy_B: Callable,
    k_tile_cnt: Int32,
) -> PipelineState:
...
def GemmSm100_mma(
    self,
    ab_pipeline: PipelineAsync,
    acc_pipeline: PipelineAsync,
    ab_consumer_state: PipelineState,
    acc_producer_state: PipelineState,
    tiled_mma: cute.TiledMma,
    tCrA: cute.Tensor,
    tCrB: cute.Tensor,
    acc: cute.Tensor,
    k_tile_cnt: Int32,
) -> tuple[PipelineState, PipelineState, cute.TiledMma]:
...
def GemmSm100__compute_stages(
    self,
) -> tuple[int, int, int]:
    smem_capacity = cutlass.utils.get_smem_capacity_in_bytes("sm_100")
    epi_stage = 4 if cute.size(self.epi_tile[1]) <= 16 else 2
    ...
    ab_bytes_per_stage = cute.size_in_bytes(self.a_dtype, a_smem_layout_staged_one) + cute.size_in_bytes(self.b_dtype, b_smem_layout_staged_one)
    mbar_helpers_bytes = 1024
    epi_bytes_per_stage = cute.size_in_bytes(self.d_dtype, d_smem_layout_staged_one)
    ...
    return ab_stage, 2, epi_stage

def GemmSm100__setup_attributes(self):
    ...
    self.ab_stage, self.num_acc_stage, self.epi_stage = self._compute_stages()
...
@staticmethod
def GemmSm100__make_tma_epi_atoms_and_tensors(
    mD: cute.Tensor,
    epi_smem_layout_staged: cute.ComposedLayout,
    epi_tile: tuple[cute.Layout, cute.Layout],
) -> tuple[cute.CopyAtom, cute.Tensor]:
    epi_smem_layout = cute.slice_(epi_smem_layout_staged, (None, None, 0))
    d_cta_v_layout = cute.composition(cute.make_identity_layout(mD.shape), epi_tile)
    op = cpasync.CopyBulkTensorTileS2GOp()
    return cpasync.make_tiled_tma_atom(
        op, mD, epi_smem_layout, d_cta_v_layout,
    )

def GemmSm100_epilog_tmem_copy_and_partition(
    self,
    tidx: Int32,
    tAcc: cute.Tensor,
    epi_tile: cute.Tile,
    ...
    cAcc = cute.make_identity_tensor((self.mma_tiler[0], self.mma_tiler[1]))
...
def GemmSm100_epilog_smem_store_and_partition(
    self,
...
def GemmSm100_epilog_gmem_copy_and_partition(
    self,
    ...
    src_tensor, dst_tensor = sD, tDgD_for_tma_partition
...
def GemmSm100_epi_load_acc_subtile(
    self,
    tiled_copy_t2r: cute.TiledCopy,
...
@cute.jit
def GemmSm100_epilogue(
    self,
    epi_store_pipeline: PipelineAsync,
    epi_read_state: PipelineState,
    epi_tile: cute.Tile,
    ...
    copy_D: Callable,
    tile_coord_mnkl: cute.Coord,
    epilogue_barrier: NamedBarrier,
    tile_scheduler,
    tidx: Int32,
    is_tma_warp: Boolean,
) -> PipelineState:
    epi_tile_shape = cute.zipped_divide(
        cute.make_layout(self.mma_tiler[:2]), epi_tile
    ).shape[1]
    ...
    return epi_read_state

@cute.kernel
def GemmSm100_kernel(
    self,
    tiled_mma: cute.TiledMma,
    ...
    if warp_idx == self.ab_load_warp_id:
        ...
        while work_tile.is_valid_tile:
            ...
            k_tile_cnt = cute.ceil_div(len_k, self.mma_tiler[2])
    ...
    if warp_idx < self.mma_warp_id:
        ...
        epi_tidx = tidx
        tiled_copy_t2r, tTR_tAcc_base, tTR_rAcc = self.epilog_tmem_copy_and_partition(epi_tidx, tCtAcc_base, epi_tile)
...
class GemmSm100:
    ...
    make_ab_pipeline = GemmSm100_make_ab_pipeline
    make_acc_pipeline = GemmSm100_make_acc_pipeline
    make_epi_store_pipeline = GemmSm100_make_epi_store_pipeline
    _compute_stages = GemmSm100__compute_stages
    _compute_num_tmem_alloc_cols = GemmSm100__compute_num_tmem_alloc_cols
    load_AB = GemmSm100_load_AB
    mma = GemmSm100_mma
    _setup_attributes = GemmSm100__setup_attributes
    __call__ = GemmSm100___call__
    _make_tma_epi_atoms_and_tensors = GemmSm100__make_tma_epi_atoms_and_tensors
    kernel = GemmSm100_kernel
    epilog_tmem_copy_and_partition = GemmSm100_epilog_tmem_copy_and_partition
    epilog_smem_store_and_partition = GemmSm100_epilog_smem_store_and_partition
    epilog_gmem_copy_and_partition = GemmSm100_epilog_gmem_copy_and_partition
    epi_load_acc_subtile = GemmSm100_epi_load_acc_subtile
    epilogue = GemmSm100_epilogue
